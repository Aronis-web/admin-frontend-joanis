  ...
  createRole: async (data: CreateRoleRequest): Promise<CreateRoleResponse> => {
    try {
      ...
      if (error.response?.status === 400) {
        ...
        throw new Error(`Datos inválidos: ${error.response?.data?.message || 'Verifica los datos del rol'}`);
      }
      if (error.response?.status === 422) {

        const validationErrors = error.response?.data?.errors || error.response?.data?.message || 'Error de validación';
...
export const rolePermissionsApi = {
  ...
  assignPermissionsToRole: async (roleId: string, permissionKeys: string[]): Promise<void> => {
    try {
      ...
      if (error.response?.status === 400) {
        throw new Error(`Datos inválidos: ${error.response?.data?.message || 'Verifica los permisos seleccionados'}`);
      }
      if (error.response?.status === 422) {
...
export const userPermissionsApi = {
  ...
  getUserPermissions: async (userId: string): Promise<{permission_key: string, granted: boolean}[]> => {
    try {
