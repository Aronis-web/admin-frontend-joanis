...
import {
  useSireVentasDeclaredInvoices,
  useSyncSireVentasDeclared,
} from '@/hooks/api/useSireVentasDeclared';
import type {
  GetSireVentasDeclaredInvoicesParams,
  SireVentasDeclaredInvoiceListItem,
} from '@/types/sireVentasDeclared';
import { formatDateToString } from '@/utils/dateHelpers';
import Alert from '@/utils/alert';

type Props = NativeStackScreenProps<any, 'SireVentasDeclared'>;
...
const formatSerieNumero = (item: SireVentasDeclaredInvoiceListItem) => {
...
export const SireVentasDeclaredScreen: React.FC<Props> = ({ navigation: _navigation }) => {
  ...
  // ============ Query params ============
  const params: GetSireVentasDeclaredInvoicesParams = useMemo(() => {
    const p: GetSireVentasDeclaredInvoicesParams = {
      limit,
      offset: (page - 1) * limit,
    };
  ...
  const { data, isLoading, isFetching, refetch, isError, error } =
    useSireVentasDeclaredInvoices(params);
  const syncMutation = useSyncSireVentasDeclared();
  ...
  }, [debouncedDoc, debouncedSearch, estado, fechaFrom, fechaTo, moneda, periodo, tipoCpe]);
  ...
  const handleSync = useCallback(async () => {
    try {
      ...
      Alert.alert(
        ...
        'Se disparó la descarga del registro declarado (RVIE) de SUNAT. Podrás ver el avance en el historial de corridas.'
      );
    } catch (e: any) {
      const msg =
  ...
  const renderItem = (item: SireVentasDeclaredInvoiceListItem) => {
          ...
          <View style={styles.headerTop}>
            <View style={styles.headerTitleContainer}>
              <View style={styles.headerIconRow}>
                ...
                <Text style={styles.headerTitle}>Ventas declaradas a SUNAT</Text>
              </View>
