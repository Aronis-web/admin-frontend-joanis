...
const CONCILIATION_OPTIONS: Array<{ label: string; value: SireConciliation }> = [
  { label: 'Todos', value: 'all' },
...
const SORT_OPTIONS: Array<{ label: string; value: SireInvoiceSortBy }> = [
...
export const SireComprasScreen: React.FC<Props> = ({ navigation: _navigation }) => {
  ...
  const [conciliation, setConciliation] = useState<SireConciliation>('all');
  ...
  const [sortBy, setSortBy] = useState<SireInvoiceSortBy>('fechaEmision');
  const [sortDir, setSortDir] = useState<SireSortDir>('DESC');
  ...
  // ============ Query params ============
  const params: GetSireInvoicesParams = useMemo(() => {
    const p: GetSireInvoicesParams = {
      limit,
      offset: (page - 1) * limit,
      sortBy,
      sortDir,
  ...
  const renderItem = (item: SireInvoiceListItem) => {
        ...
        <View style={styles.itemFooter}>
          <View style={styles.footerBadge}>
            ...
            <Caption color={theme.color.text.muted}>
              {item.linksCount} compra{item.linksCount === 1 ? '' : 's'} anexada
              {item.linksCount === 1 ? '' : 's'}
            </Caption>
          </View>
          <View style={styles.footerBadge}>
    ...
    <ScreenLayout navigation={_navigation as any}>
      <SafeAreaView style={styles.safeArea} edges={['top']}>
        ...
        >
          ...
          <View style={styles.chipsSection}>
            ...
            <ChipGroup
              ...
              onChange={(sel) => {
                const v = (sel[0] as SireConciliation) || 'all';
                setConciliation(v);
                setPage(1);
              }}
              variant="filled"
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing[2] }}>
                  <View style={{ flex: 1 }}>
                    <ChipGroup
                      ...
                      selected={[sortBy]}
                      onChange={(sel) => {
                        setSortBy((sel[0] as SireInvoiceSortBy) || 'fechaEmision');
                      }}
                      variant="filled"
                      size="small"
                    />
        ...
        <SunatReportModal
          ...
          title="Descargar reporte · Compras"
          defaultDatasets={['compras-mapeadas']}
          fileBaseName="reporte-sire-compras"
        />
