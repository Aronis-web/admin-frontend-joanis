...
export const useTenantStore = create<TenantState>((set, get) => ({
  ...
  initTenantContext: async () => {
    try {
      ...
      // First, try to load from tenant-specific storage
      ...
      // Parse company JSON with error handling
      if (companyJson) {
        try {
      ...
      // Parse site JSON with error handling
      if (siteJson) {
        try {
      ...
      // Parse warehouse JSON with error handling
      if (warehouseJson) {
        try {
      ...
      // If tenant store is empty, sync from auth store (fallback)
      if (!selectedCompany || !selectedSite) {
        ...
        if (!selectedCompany && authCompanyJson) {
          try {
            ...
            // Save to tenant store for future use
